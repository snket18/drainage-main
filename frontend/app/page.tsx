...
import { Activity, ShieldAlert, Navigation, Video } from 'lucide-react';
  ...
  return (
    <div className="relative w-full h-[calc(100vh-2rem)] border border-slate-300 bg-slate-50 flex flex-col">
       ...
       <div className="absolute inset-0 z-0">
          <InteractiveMap heightClass="h-full w-full" />
       </div>
                   ...
                   <ul className="space-y-2">
                     {selectedArea.riskDrivers.map((risk, i) => (
                        <li key={i} className="text-xs">
                          <span className="font-bold text-slate-900 block">{risk.factor}</span>
                          <span className="text-slate-600">{risk.detail}</span>
                        </li>
                     ))}
                   </ul>
                </div>
       ...
       <div className="absolute top-28 right-4 bottom-4 z-10 w-[340px] flex flex-col gap-2 pointer-events-none overflow-y-auto scrollbar-none pb-4">
          
          {/* Priority Advisory */}
          {stepState.activeAlert && (
             <div className="bg-white border border-slate-300 p-4 pointer-events-auto shrink-0">
                <div className="flex items-center justify-between border-b border-slate-200 pb-2 mb-3">
                   <div className="flex items-center gap-2 font-bold text-xs text-rose-700">
                     <ShieldAlert className="w-4 h-4" />
                     <span>Advisory</span>
                   </div>
                   <span className="text-[10px] font-bold text-slate-600 uppercase tracking-wider bg-slate-200 px-2 py-0.5">
                     {stepState.phase}
                   </span>
                </div>
                ...
                <div className="bg-slate-100 p-3 border border-slate-200 mb-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                   <Link href="/interventions" className="flex items-center justify-center bg-slate-900 hover:bg-slate-800 text-white py-2 text-xs font-semibold">
                     <span>Interventions</span>
                   </Link>
                   <Link href="/routing" className="flex items-center justify-center gap-1.5 bg-white hover:bg-slate-100 text-slate-700 py-2 text-xs font-semibold border border-slate-300">
...
function StatPanel({ title, value, unit }: { title: string, value: string | number, unit: string }) {
  return (
    <div className="flex justify-between items-center bg-white border border-slate-300 p-3 pointer-events-auto">
      <div className="text-xs uppercase tracking-widest text-slate-600 font-bold">{title}</div>
